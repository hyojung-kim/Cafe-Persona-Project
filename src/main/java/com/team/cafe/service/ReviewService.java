...
public class ReviewService {
    ...
    public ReviewService(CafeRepository cafeRepository,
    ...
    public Review createReview(Long cafeId,
        ...
        if (imageUrls != null) {
            ...
            for (String url : imageUrls) {
                ...
                review.addImage(img); // <-- 이 메서드가 img.setReview(this) + 컬렉션 add + sort 보정 수행
            }
        }
    ...
    public Review updateReview(Long reviewId,
        ...
        boolean isAdmin = editor.getRoles().stream().anyMatch(r -> "ROLE_ADMIN".equalsIgnoreCase(r));
        if (!isAuthor && !isAdmin) {
        ...
        // 기존 이미지 전량 제거 (orphanRemoval=true 이므로 FK 안전하게 제거됨)
        if (review.getImages() != null && !review.getImages().isEmpty()) {
            // 안전하게 복사 후 제거
            List<ReviewImage> copy = new ArrayList<>(review.getImages());
            for (ReviewImage img : copy) {
                review.removeImage(img); // 내부에서 컬렉션 제거 + img.review = null + sort 재정렬
            }
        }
