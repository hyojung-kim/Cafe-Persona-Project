...
- 별점이 저장되지 않음
- 리뷰 수정/삭제가 안 됨
- 리뷰 목록이 안 보이거나 일부만 보임
- 리뷰 이미지 업로드 오류
- 중복 리뷰 등록
- 작성 직후 UI 반영이 느림
  
✅ **문제 해결**
- 로그인/권한 확인, 요청 payload(내용/별점) 누락 점검, API 4xx 메시지로 원인 파악
- 프론트 상태 동기화 확인, 백엔드 유효성(1~5) 검증 로그 확인, DB 컬럼 타입/범위 점검
- 작성자 일치 체크 로직 확인, 권한 미들웨어 조건 점검, soft delete 설정 여부 확인
- 페이징/정렬 파라미터 점검, 필터 조건 충돌 확인, 캐시 갱신/무효화 처리 확인
- 파일 크기/확장자 제한 확인, multipart 헤더와 폼데이터 구성 점검, 저장 경로/퍼미션 확인
- 동일 사용자·상품 중복 방지 로직 확인, 유니크 인덱스/서비스 레벨 체크 추가
- optimistic update/리패치 트리거 추가, 캐시 invalidate 처리 확인
<br><br>
---
