...
keyword 타입별 AND매칭 구현, 기존 로직에서 SQL을 어떻게 확장하는 것이 좋을지 고민을 많이하게됨 <br>
...
결론적으로 where + orderby + groupby + having 을 전부 사용하기 때문에 커스텀을 위해 JPQL에서 Native Query방식으로 변경, 리팩토링 진행  <br>
파라미터값이 다중선택으로 목록으로 전달이 됨, 기존 sql로직도 동작, 페이지 변경, 카페주소 검색을 해도 일관성있게 동작함 ! <br>
